          ...
          {tabs.map((tab) => (
            <div key={tab.id}>
              <button
                ...
                className={`${
                  activeTab === tab.id ? "text-white bg-black" : "text-black"
                } text-base font-medium px-4 py-1.5 rounded`}
              >
                {tab.title}
              </button>
            ...
            <div className="w-full">
              ...
              <div className="bg-white border-2 border-black px-6 pt-1.5 rounded-md shadow-lg">
                {websites.map((website) => (
                  <div key={website.id}>
                    <div className="flex justify-between items-center mt-1 py-2.5 px-0 mb-2 rounded  transition-all ">
                      ...
                      >
                        <p>View site</p>{" "}
                        <FaArrowRightLong className="h-5 w-5 flex-shrink-0 ml-3 group-hover:-rotate-45 transition-all" />
                      </a>
                    </div>
                    {website.id !== websites.length - 1 && (
              ...
              {creatives.map((image, index) => (
                <div key={index}>
                  <img
                    src={image}
                    alt="Creative"
                    className="h-full w-full aspect-[6/6] object-fit rounded border-4 border-white hover:border-black hover:shadow-2xl transition-all"
                  />
                </div>
              ))}
            </div>

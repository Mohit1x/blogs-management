interface FormData {
    title: string;
    tags: string[]; // Correct: tags is an array of strings
}
    ...
    const fetchBlog = async () => {
        ...
        if (!id || id === ":id") return;
            const  {data}  = await axios.get(`http://localhost:5000/api/blogs/blog/${id}`);
                ...
                console.log(data.blog.tags)            
            if (data) {
                console.log("Founded Blog: ", data)
                const parsedTags = JSON.parse(data.blog.tags)
                console.log(parsedTags,"actualtags")
                setFetchedBlog(data.blog)
                setFormData({ title: data.blog.title, tags: parsedTags })
                setTags(parsedTags)
                setFetchedBlogContent(data.blog.content)
                if (data.blog.image) {
                    setImage(data.blog.image)
                    setPreview(`http://localhost:5000/uploads/${data.blog.image}`);
                ...
                let updatedTags = [...parsedTags, ...tags];
